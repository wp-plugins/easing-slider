...
wp_register_script('script', $scripturl.'script.js', '', '1.0');
      ...
      <?php $recent = new WP_Query('cat='.$featcat.'&showposts='.$featpost.'');
        while($recent->have_posts()) : $recent->the_post(); global $post;
          $image = get_post_meta($post->ID, 'easing', true); if (!empty($image)) { ?>
                  <li><a href="<?php the_permalink(); ?>"><img src="<?php echo $image; ?>" style="width:<?php echo $width; ?>px;" alt="<?php echo $images; ?>" /></a></li>
...
<tr valign="top" style="border-bottom:1px solid #ccc;"><td style="padding:5px 0;"><label for="interval" style="padding:10px;font-weight:bold;">transitionSpeed:</label></td>
		...
		<tr valign="top" style="border-bottom:1px solid #ccc;"><td style="padding:5px 0;"><label for="transpeed" style="padding:10px;font-weight:bold;">autoPlay:</label></td>
